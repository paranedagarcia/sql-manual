...
Un ejemplo de un script (guión) SQL se muestra en el siguiente bloque de código:


```sql showLineNumbers title="Sentencia SQL para selección de datos"
...
Donde: 
* **`SELECT`** permite elegir que campos (o columnas como nombre, apellido, rut, edad y genero) queremos mostrar, 
* **`FROM`** nos indica desde que tabla (clientes) y donde están definidos los campos. 
* **`WHERE`** permite especificar una condición (o filtro que queremos ejecutar sobre los datos resultantes) y 
* **`ORDER BY`** nos permite obtener una lista ordenada por edad en forma ascendente.
...
<figcaption>Mapa de visualización de las áreas de uso de SQL.</figcaption>
</figure>
</center>
...
```sql showLineNumbers title="Uso de 'AS NUMERIC' para forzar el tipo de datos."
SELECT CAST(12345.12 AS NUMERIC(10,5)) --retorna 12345.12000
```

**Numéricos**

Los de tipo numéricos incluyen enteros y decimales con un almacenamiento desde -32768 a +32767 para "*smallint*", y hasta +9223372036854775807 para un "*bigint*"
