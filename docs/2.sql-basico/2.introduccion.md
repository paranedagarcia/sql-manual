...
La diferencia principal entre un **Data Owner** (dueño del dato) y un **Data Steward** (custodio de datos) radica en el tipo de responsabilidad que tienen sobre los datos.
<center>
![](img/data-owner.webp)
</center>

* **Data Owner:** es el **responsable del negocio** por un conjunto de datos. Decide qué datos son importantes, quién puede acceder a ellos, cómo deben utilizarse y qué nivel de calidad se espera. Tiene autoridad para tomar decisiones estratégicas sobre esos datos.

* **Data Steward:** es el **responsable operativo** de que esas decisiones se implementen correctamente. Se preocupa por mantener la calidad, consistencia, documentación y cumplimiento de las políticas definidas por el Data Owner.

En términos simples:

> **El Data Owner define "qué" y "por qué"; el Data Steward asegura "cómo" se cumple.**

#### Comparación

| Aspecto            | Data Owner                           | Data Steward                                        |
| ------------------ | ------------------------------------ | --------------------------------------------------- |
| Enfoque            | Negocio                              | Operación y gestión                                 |
| Responsabilidad    | Rendición de cuentas sobre los datos | Administración diaria de los datos                  |
| Nivel              | Estratégico                          | Táctico y operativo                                 |
| Toma de decisiones | Sí                                   | Normalmente no (ejecuta y recomienda)               |
| Calidad de datos   | Define el nivel requerido            | Monitorea y mejora la calidad                       |
| Accesos            | Autoriza quién puede acceder         | Implementa y controla los accesos según políticas   |
| Documentación      | Aprueba estándares                   | Mantiene metadatos, glosarios y catálogos           |
| Indicadores        | Evalúa impacto en el negocio         | Mide calidad, completitud, consistencia, actualidad |
...
El Data Owner suele ser un gerente o jefe del área de negocio que utiliza los datos para tomar decisiones.

Sus funciones incluyen:

* Definir el propósito de los datos.
* Aprobar políticas de uso.
...
* Definir niveles aceptables de calidad y de seguridad de la información.
* Asumir la responsabilidad frente a auditorías.
* Priorizar iniciativas de mejora de datos.
* Resolver conflictos entre distintas áreas.

Ejemplos:

* Gerente Comercial → propietario de los datos de clientes.
* Director de Finanzas → propietario de datos financieros.
* Director de Recursos Humanos → propietario de datos de empleados.
...
El Data Steward trabaja más cerca de la operación diaria.
...
* Supervisar indicadores de calidad.
* Coordinar con TI y las áreas de negocio.
* Capacitar a usuarios sobre buenas prácticas.

No suele decidir la política, sino implementarla y recomendar mejoras.

#### Ejemplo práctico

Supongamos una institución pública que administra un registro de ciudadanos.

#### Data Owner

La Directora del Registro Civil podría decidir que:

* Todo ciudadano debe tener un RUN único.
* La fecha de nacimiento es obligatoria.
* Sólo ciertos funcionarios pueden modificar el domicilio.
* La calidad mínima permitida es 99,8 %.

#### Data Steward

El Data Steward implementa estas decisiones:

* Detecta RUN duplicados.
* Busca domicilios incompletos.
* Monitorea registros sin fecha de nacimiento.
* Actualiza el catálogo de datos.
* Genera reportes mensuales de calidad.
* Coordina la corrección con las áreas responsables.

#### Analogía más sencilla

Pensemos en una biblioteca.

**El Data Owner** es el director de la biblioteca:

* Decide qué libros comprar.
* Define las reglas de préstamo.
* Autoriza presupuestos.
* Es responsable ante la institución.

**El Data Steward** es el bibliotecario:

* Ordena los libros.
* Corrige errores en el catálogo.
* Mantiene el inventario.
* Verifica que se cumplan las normas.

Uno **gobierna**; el otro **administra**.
...
```text
             ...
             Data Owner
                  │
       Define políticas y reglas
                  │
                  ▼
            Data Steward
                  │
 Implementa controles y monitorea
                  │
                  ▼
      Equipos de TI / Ingeniería
                  │
                  ▼
 Bases de datos, Data Lake, Data Warehouse
```


#### Resumen de relación

La diferencia esencial es la **rendición de cuentas**:

* **Data Owner:** tiene la **responsabilidad final (accountability)** sobre un dominio de datos y toma decisiones de negocio.
* **Data Steward:** tiene la **responsabilidad operativa (responsibility)** de aplicar esas decisiones, mantener la calidad de los datos y asegurar el cumplimiento de las políticas.

En marcos de gobierno de datos como DAMA-DMBOK, COBIT o ISO/IEC 38505, ambos roles son complementarios: un gobierno de datos eficaz requiere que el Data Owner establezca la dirección y que el Data Steward la traduzca en prácticas y controles operativos.


### Arquitecto de Datos (Data Architect)

<center>
![](img/data-architect.webp)
</center>

Es tal la complejidad actual de los datos respecto de sus orígenes así como el volumen de datos que requieren ser transados, que se hace necesaria la participación de un nuevo perfil, el **arquitecto de datos** (**DA**). Por sobre los perfiles mencionados anteriormente, es el DA el encargado de diseñar y orquestar las plataformas necesarias para el procesamiento masivo de datos para su transformación en información. Es un perfil netamente operativo encargado de construir y mantener la infraestructura de los datos de la organización.
...
<center>
![](img/data-enginner.webp)
</center>

El trabajo del ingeniero de datos (**DE**) es "la representación y el movimiento de datos para que sean consumibles y utilizables", dijo Pope. El ingeniero de datos, debe poder tomar los datos sin procesar, limpiarlos, moverlos a una base de datos, etiquetarlos y, en general, asegurarse de que estén listos para la siguiente etapa del proceso.
...
### Perfiles en el Gobierno de Datos

Una distribución habitual de responsabilidades quedaría de la siguiente manera:

| Rol                      | Principal responsabilidad                                           |
| ------------------------ | ------------------------------------------------------------------- |
| Chief Data Officer       | Define la estrategia de datos de la organización.                   |
| Data Owner               | Es responsable de los datos desde la perspectiva del negocio.       |
| Data Steward             | Gestiona la calidad, metadatos y cumplimiento operativo.            |
| Data Custodian           | Administra la infraestructura técnica donde residen los datos (bases de datos, respaldos, seguridad, disponibilidad). |
| Data Engineer            | Construye y mantiene los procesos de integración y transformación de datos (ETL/ELT, pipelines).  
